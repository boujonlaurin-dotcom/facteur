# Story 2.2c : Onboarding avec Sous-Thèmes (MAJ)

> **Parent Story**: [[./2.2c.onboarding-section3.md]]  
> **Type**: Evolution
...
**I want** préciser mes intérêts via sous-thèmes lors de l'onboarding,  
**so that** mes recommendations soient plus précises dès le départ.

## Background

Modification de la Story 2.2c existante pour intégrer les sous-thèmes granulaires. L'UX proposée utilise des "tags enfants" qui apparaissent sous chaque thème macro sélectionné.

## Acceptance Criteria

1. **UX "Tags Enfants"** : Au clic sur un thème macro, 3-4 sous-thèmes apparaissent comme tags cliquables
2. **Multi-sélection optionnelle** : User peut sélectionner 0-N sous-thèmes (pas obligatoire)
3. **UI fluide** : Transitions douces, max 2 lignes de tags visuellement distincts
4. **Backend API modifiée** : Accepter `subtopics` en plus de `themes` dans `OnboardingAnswers`
...
class _ThemeWithSubtopicsState extends State<ThemeWithSubtopics> {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        // Thème principal (card cliquable)
        GestureDetector(
        ...
        // Sous-thèmes (tags enfants, apparaissent si thème sélectionné)
...
class AvailableSubtopics {
  static final Map<String, List<SubtopicOption>> byTheme = {
    'tech': [
      ...
      SubtopicOption(slug: 'cybersecurity', label: 'Cybersécurité'),
    ],
    'society': [
      SubtopicOption(slug: 'social-justice', label: 'Justice sociale'),
      SubtopicOption(slug: 'health', label: 'Santé'),
      SubtopicOption(slug: 'education', label: 'Éducation'),
      SubtopicOption(slug: 'housing', label: 'Logement'),
    ],
    'environment': [
      SubtopicOption(slug: 'climate', label: 'Climat'),
      SubtopicOption(slug: 'biodiversity', label: 'Biodiversité'),
      SubtopicOption(slug: 'energy-transition', label: 'Transition énergétique'),
    ],
    // ... 5 autres thèmes avec 3-4 subtopics chacun
  };
}
```
...
class OnboardingAnswers {
  ...
  final List<String>? subtopics;  // ← NOUVEAU
  // ... other fields
...
class OnboardingAnswers(BaseModel):
    ...
    subtopics: Optional[list[str]] = Field(default_factory=list, description="Topics granulaires sélectionnés")  # ← NOUVEAU
    # ...
```
...
async def save_onboarding(self, user_id: str, answers: OnboardingAnswers) -> dict:
    # ... existing logic pour themes → user_interests
    ...
    return {
        ...
        "subtopics_created": subtopic_count,  # ← NOUVEAU
...
```
┌─────────────────────────────────────────┐
│ Question : Tes thèmes d'intérêt ?       │
├─────────────────────────────────────────┤
│                                         │
│  [💻 Tech & Innovation]  ← sélectionné │
│    └─ #IA  #Crypto  #Espace  #Cyber    │  ← Tags enfants (3-4 max)
│                                         │
│  [👥 Société & Vie]  ← non sélectionné │
│                                         │
│  [🌿 Environnement & Climat]  ← sélect.│
...
testWidgets('Sous-thèmes apparaissent au clic thème', (tester) async {
  ...
  // Vérifier tags enfants visibles
  expect(find.byType(SubtopicChip), findsNWidgets(4));
});

testWidgets('Sélection sous-thème ajoute au state', (tester) async {
  // ...
...
- **Priority :** 🟢 MEDIUM (améliore UX mais pas bloquant)
- **Sprint :** +2 (parallèle à 4.1d)

## Dependencies

- **Bloqué par :** Story 4.1c (table `user_subtopics` doit exister)
- **Parallèle avec :** Story 4.1d (classification)

## Change Log

| Date | Version | Description | Author |
|------|---------|-------------|--------|
| 19/01/2026 | 2.0 | MAJ avec sous-thèmes granulaires (UX tags enfants) | Antigravity |
