...
### PR-4 — Backend cleanup [absorbé par PR-2]

> **Statut** : ⚠️ **Absorbé par PR-2** — les modules LLM (`digest_builder`,
> `topic_suggester`, `source_suggester`, `scheduling`, `veille_generation_job`)
> importaient `VeilleDelivery` / `VeilleFrequency`, qui ont été retirés des
> modèles en PR-2. Les conserver aurait cassé la collecte pytest. L'adoption
> gate (« attendre adoption mobile ≥ 95 % ») ne s'appliquait pas à ces modules
> internes — seuls les endpoints publics demandaient un shim (cf. R5), traité
> via 410 Gone également en PR-2.
>
> **Fichiers supprimés en PR-2** :
...
> - Tests associés (6 fichiers, ~1 775 lignes).
