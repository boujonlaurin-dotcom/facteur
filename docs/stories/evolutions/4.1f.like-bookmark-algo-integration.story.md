...
**so that** le contenu proposé correspond de mieux en mieux à mes centres d'intérêt réels.

## Status: Done

## Acceptance Criteria

1. [x] **Bouton Like** : Un bouton coeur est disponible sur les cartes du feed et du digest
2. [x] **Like Backend** : `POST /contents/{id}/like` et `DELETE /contents/{id}/like` créent/suppriment le like
3. [x] **Like DB** : Colonnes `is_liked` (bool) et `liked_at` (timestamp) ajoutées à `user_content_status`
4. [x] **Subtopic Weight Boost (Like)** : Liker un article booste les `user_subtopics.weight` des topics associés (+0.15)
5. [x] **Subtopic Weight Boost (Bookmark)** : Sauvegarder un article booste les `user_subtopics.weight` (+0.05)
6. [x] **Weight Caps** : Poids plafonné à 3.0, plancher à 0.1
7. [x] **Scoring Engine** : `ArticleTopicLayer` utilise les poids pondérés (`TOPIC_MATCH * weight`) au lieu d'un score fixe
8. [x] **Score Transparency** : L'explication de scoring affiche "Renforcé par vos j'aime" quand un topic a un poids > 1.0
9. [x] **Digest Actions** : Actions `LIKE` et `UNLIKE` ajoutées au `DigestAction` enum et traitées dans `DigestService`
10. [x] **Feed Card UX** : Les cartes du feed affichent 4 boutons compacts : Like, Save, Voir moins, Info
11. [x] **Digest / Feed cohérence** : Le boost subtopic s'applique via les deux chemins (feed + digest)
...
- [x] **Task 1: Migration Alembic** (AC: 3)
  - [x] Ajouter `is_liked BOOLEAN NOT NULL DEFAULT false` à `user_content_status`
  - [x] Ajouter `liked_at TIMESTAMPTZ` à `user_content_status`
  - [x] Créer index composite `ix_user_content_status_user_liked` sur `(user_id, is_liked)`
  - Migration: `b3c4d5e6f7a8_add_is_liked_to_user_content_status.py`

### Backend — Model & Schema

- [x] **Task 2: Modèle ORM** (AC: 3)
  - [x] Ajouter `is_liked: Mapped[bool]` et `liked_at: Mapped[Optional[datetime]]` à `UserContentStatus`
  - Fichier: `packages/api/app/models/content.py`

- [x] **Task 3: Schémas Pydantic** (AC: 2, 9)
  - [x] Ajouter `is_liked: bool = False` à `ContentResponse` et `ContentDetailResponse`
  - [x] Ajouter `is_liked: bool = False` à `DigestItem`
  - [x] Ajouter `LIKE = "like"` et `UNLIKE = "unlike"` à `DigestAction` enum
  - Fichiers: `packages/api/app/schemas/content.py`, `packages/api/app/schemas/digest.py`

### Backend — Service

- [x] **Task 4: ContentService** (AC: 2, 4, 5, 6)
  - [x] `set_like_status(user_id, content_id, is_liked)` — upsert like/unlike avec delta +/-0.15
  - [x] `_adjust_subtopic_weights(user_id, content_id, delta)` — méthode générique : charge topics du contenu, ajuste `user_subtopics.weight` avec cap [0.1, 3.0]
...
- [x] **Task 6: Endpoints Like** (AC: 2)
  - [x] `POST /contents/{content_id}/like` — marque comme aimé
  - [x] `DELETE /contents/{content_id}/like` — retire le like
...
- [x] **Task 9: ArticleTopicLayer** (AC: 7, 8)
  - [x] Scoring pondéré : `TOPIC_MATCH * weight` au lieu de `TOPIC_MATCH` fixe
  - [x] Annotation `[liked: topic]` quand `weight > 1.0` pour label "Renforcé par vos j'aime"
...
- [x] **Task 10: Hydratation poids** (AC: 7)
  ...
  - [x] Label "Renforcé par vos j'aime" dans `_determine_top_reason`
...
- [x] **Task 11: Content Model** (AC: 1)
  - [x] Ajouter `isLiked` au modèle `Content` (constructeur, fromJson, copyWith)
  - Fichier: `apps/mobile/lib/features/feed/models/content_model.dart`

- [x] **Task 12: Feed Repository** (AC: 2)
  - [x] `toggleLike(contentId, isLiked)` — appels `POST`/`DELETE /contents/{id}/like`
  - Fichier: `apps/mobile/lib/features/feed/repositories/feed_repository.dart`

- [x] **Task 13: Feed Provider** (AC: 1)
  - [x] `toggleLike(Content)` — mise à jour optimiste
  - Fichier: `apps/mobile/lib/features/feed/providers/feed_provider.dart`

- [x] **Task 14: Feed Card UX** (AC: 1, 10)
  - [x] Bouton Like (coeur rempli si `isLiked`, couleur terracotta)
  - [x] Bouton Save (rempli si `isSaved`)
  - [x] Bouton Voir moins (`onNotInterested`)
  - [x] Bouton Info (icône compacte, remplace l'ancien texte "Personnalisation")
  - Fichier: `apps/mobile/lib/features/feed/widgets/feed_card.dart`

- [x] **Task 15: Feed Screen Wiring** (AC: 1, 10)
  - [x] Passer `onLike`, `isLiked`, `onSave`, `isSaved`, `onNotInterested`, `onPersonalize` à FeedCard
  - Fichier: `apps/mobile/lib/features/feed/screens/feed_screen.dart`

- [x] **Task 16: Digest Models** (AC: 9)
  - [x] Ajouter `isLiked` à `DigestItem` (Freezed, + .freezed.dart + .g.dart)
...
- [x] **Task 17: Digest Provider & Screen** (AC: 9)
  ...
  - [x] `_convertToContent` passe `isLiked` et `isSaved` au modèle `Content`
  - Fichiers: digest_screen.dart, digest_provider.dart, digest_briefing_section.dart

- [x] **Task 18: Saved Screen** (AC: 10)
  - [x] Passer `isLiked: content.isLiked` à FeedCard
  - [x] Retirer le callback noop `onPersonalize: () {}`
  - Fichier: `apps/mobile/lib/features/saved/screens/saved_screen.dart`

### Tests

- [x] **Task 19: Tests Backend**
  - [x] 7 tests: like/unlike, subtopic weight +0.15/-0.15, weight cap 3.0, bookmark boost +0.05, création nouveau subtopic
  - Fichier: `packages/api/tests/test_like_feature.py`

- [x] **Task 20: Tests Frontend**
...
Le mécanisme de "like" est séparé du statut existant (`unseen/seen/consumed/saved/hidden`). `is_liked` est un **flag orthogonal** sur `user_content_status`, pas un nouveau statut — un article peut être à la fois "consumed" et "liked".
...
Les deux chemins appellent `_adjust_subtopic_weights()` pour garantir la cohérence.

### Formule de Scoring Modifiée
...
Quand `weight > 1.0`, le label "Renforcé par vos j'aime" apparaît dans l'explication de scoring.
...
| `LIKE_INTEREST_RATE` | 0.03 | Réservé usage futur |
...
| `apps/mobile/lib/features/feed/widgets/feed_card.dart` | +Like btn, compact info btn |
...
|------|---------|-------------|--------|
| 15/02/2026 | 1.0 | Implementation complète — 20 tasks done | Dev Agent (Claude) |
