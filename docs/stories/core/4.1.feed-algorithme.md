# Story 4.1: Algorithme de tri et personnalisation [DÉPRÉCIÉ]

> ⚠️ **ATTENTION : Cette story est dépréciée et remplacée par [Story 4.2 - Recommendation Engine V3](./4.2.reco-engine-v3.story.md)**
>
> La nouvelle implémentation (V3) inclut :
> - Architecture 3 niveaux (47 labels + NER illimité)
> - Traitement asynchrone avec file d'attente
> - EntityLayer pour scoring par entités (+60pts)
> - mDeBERTa et spaCy en production
>
> **Date de dépréciation :** 29/01/2026

## Status: ~~Ready ✅~~ → **DÉPRÉCIÉ** 🚫

**Remplacée par :** [Story 4.2 - Recommendation Engine V3](./4.2.reco-engine-v3.story.md)
...
- **Harnais de personas** : `scripts/evaluate_scoring_personas.py` (+
  `build_persona_dataset.py` / `build_scoring_corpus.py`) — personas **dérivés de
  comptes réels** et corpus gelé, sans DB ni réseau. Remplace l'ancien
  `scripts/validate_personas.py`, qui écrivait de faux profils dans une base
  vivante. Runbook :
  [`maintenance-scoring-tuning-harness.md`](../../maintenance/maintenance-scoring-tuning-harness.md).
