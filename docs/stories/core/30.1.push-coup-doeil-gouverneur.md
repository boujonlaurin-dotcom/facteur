...
> **Amendement du 26/07 (arbitrage PO, story 30.2)** — la règle des 4h de ce
> gouverneur refusait *toute* alerte : un kind non rituel est bloqué dès qu'un
> rituel a été envoyé (ou est attendu) à moins de 4h, or l'alerte source part
> dans la même passe que la tournée. Les alertes sont donc **exemptées du
> cooldown rituel** via le paramètre `ritual_companion=True` de
> `check_push_budget` ; les budgets 2/24h et 6/7j leur restent appliqués tels
> quels. Conséquence assumée : le budget quotidien étant partagé avec la
> tournée, au plus 1 alerte par jour passe, les suivantes sont `skipped` avec
