...
- **En profondeur / qualitatif** : parler en visio avec les utilisateurs engagés (call Calendly avec Laurin).

Le **moment de fermeture** (`ClosureScreen`, fin du digest quotidien) est le point d'ancrage : moment calme, à forte charge émotionnelle, où l'utilisateur vient de *terminer* son rituel. On y greffe les deux étages.

### Décisions actées (brainstorm)
- **Scope v1 = B** : micro-feedback emojis **+** modal d'invitation call.
- **RDV = lien Calendly externe** (pas de webhook en v1), URL en **placeholder** à remplacer avant merge.
- **Logique de déclenchement = backend** (robuste, pilotable, analysable), pas SharedPreferences seul.
- **Le micro-feedback emojis REMPLACE** le CTA Notion existant du `ClosureScreen`.
- **Gating segmenté** par niveau d'activité (cf. §2) — on cible aussi les utilisateurs qui décrochent.

### Non-objectifs v1
- Pas de webhook Calendly / mesure de conversion booking (v2).
- Pas de question ouverte conditionnelle ni de canal asynchrone permanent (v2/v3).
- Pas de WebView Calendly embarquée (on ouvre le navigateur externe).

---

## 2. Règles produit

### Micro-feedback emojis (chaque fermeture)
- 3 emojis sur le `ClosureScreen` : 😴 (mou) · 🙂 (correct) · 🔥 (top).
- 1 tap, optionnel, non bloquant. Remplace/complète le CTA Notion existant (ligne 287-329).
...
- 3 sorties :
  - **« 15 min, je suis curieux »** → ouvre Calendly (lien externe).
  - **« J'ai un truc précis à te dire »** → ouvre aussi Calendly (v1 ; champ texte direct en v2).
  - **« Pas maintenant »** → snooze 21j, fermeture sans culpabilité.
...
**Migration Alembic** : `alembic revision -m "add_feedback_tables"` → crée `digest_sentiments` + `feedback_invites`. Brancher sur le head réel (`alembic heads`), 1 seul head (hook `post-edit-alembic-heads.sh`). **Exécution SQL via Supabase SQL Editor** (jamais sur Railway, cf. CLAUDE.md) — le SQL sera fourni dans la PR.

### 3.2 Mobile (`apps/mobile`)

**Config** `lib/config/constants.dart` → `ExternalLinks.calendlyUrl`.
...
**Providers Riverpod** (`@Riverpod`) : `feedbackRepository`, `inviteStatus`.

**Widgets** :
- `lib/features/feedback/widgets/sentiment_picker.dart` — 3 emojis, intégré dans `ClosureScreen` (remplace le pill Notion ligne 287-329).
- `lib/features/feedback/widgets/call_invite_sheet.dart` — `showModalBottomSheet` (pattern `source_adjust_sheet.dart`), photo + texte + 3 boutons, ouvre `calendlyUrl` via `url_launcher` (déjà dépendance).
...
- [x] Migration Alembic `ufb01_create_feedback_tables` (1 head) + SQL Supabase dans le docstring
- [x] Tests `tests/test_feedback_router.py` (segmentation, gating should_show vrai/faux, snooze, action terminale) — 20/20 verts

### Mobile
- [x] `ExternalLinks.calendlyUrl` (placeholder)
...
- **Double-affichage** : backend = source de vérité, SharedPreferences = garde-fou session uniquement.
- **Calendly URL** : à fournir par Laurin avant merge (placeholder sinon).
...
### Mobile (`apps/mobile`)
- `lib/config/constants.dart` — `ExternalLinks.calendlyUrl` (placeholder)
...
- [ ] **Remplacer le placeholder Calendly** (`ExternalLinks.calendlyUrl`) par le vrai lien.
- [ ] **Exécuter le SQL** de `ufb01` dans le **Supabase SQL Editor** (jamais sur Railway).
