# Story 4.1c : Taxonomie 50 Topics & Normalisation Thèmes
...
**As a** développeur,  
**I want** normaliser les thèmes sources et enrichir avec 50 topics granulaires,  
**so that** le système puisse faire des recommendations précises.

## Background

Cette story **remplace Story 8.1** qui était initialement dans Epic 8 (Progression). Elle a été déplacée vers Epic 4 (Algorithme) car les sous-thèmes sont critiques pour le système de recommandation, pas uniquement pour les quiz.

## Acceptance Criteria

1. **Migration `Source.theme`** : Normalisation vers 8 valeurs fixes
   - Mapping ancien → nouveau appliqué
   - Contrainte BDD : theme IN ('tech', 'society', 'environment', 'economy', 'politics', 'culture', 'science', 'international')
2. **Table `user_subtopics`** créée avec user_id, topic_slug, weight
3. **Colonne `Content.topics TEXT[]`** ajoutée avec index GIN
4. **Les 46 sources CURATED** ont `granular_topics` renseigné manuellement
5. **Script de validation** vérifiant cohérence taxonomie (pas de topic orphelin)
...
```csv
# sources_master.csv (mise à jour pour 46 sources CURATED)

Name,URL,Type,Thème,Status,granular_topics
...
def validate():
    # 1. Vérifier que tous source.theme sont valides
    invalid_themes = session.query(Source).filter(
        ~Source.theme.in_(VALID_THEMES)
    ).all()
    assert len(invalid_themes) == 0
    
    # 2. Vérifier que tous topics dans granular_topics sont valides
    ...
    # 3. Vérifier mapping cohérent
    # ...
```

## Taxonomie 50 Topics

Voir [PRD Section Taxonomie](../prd.md#taxonomie--système-de-classification) pour la liste complète.

## Testing

1. **Migration Rollback** : Tester upgrade + downgrade sans perte de données
2. **Import sources** : Vérifier parsing JSON pour `granular_topics`
3. **Validation script** : Exécuter sur BDD de dev et prod

## Effort & Priority

- **Effort :** 8 points (1 sprint)
- **Priority :** 🟡 HIGH (requis pour Epic 8 et scoring granulaire)
- **Sprint :** +1

## Dependencies

- **Bloque :** Story 4.1d (Classification), Story 8.2-8.6 (Progression/Quiz)
- **Bloqué par :** Story 4.1b (fix bug matching)

## Change Log

| Date | Version | Description | Author |
|------|---------|-------------|--------|
| 19/01/2026 | 1.0 | Déplacée depuis Epic 8 → Epic 4 | Antigravity |
