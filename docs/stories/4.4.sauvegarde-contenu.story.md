...
## Status: In Progress 🚧 (Update Requested)
...
- [ ] **Archive on Save**:
    - [ ] Après une sauvegarde réussie, l'article disparaît automatiquement du feed principal pour simuler un "triage".
    - [ ] Un message (SnackBar) confirme l'ajout à la section "Watch Later".
    - [ ] Une option "Annuler" (Undo) est disponible dans le SnackBar pour restaurer l'article.
- [ ] **Persistence**:
    - [ ] L'état est sauvegardé via l'API.
    - [ ] L'état persiste au rechargement de l'app.
- [ ] **Data Integrity**:
    - [ ] Sauvegarder un contenu ne doit PAS effacer son statut de lecture (vu/consommé).
    - [ ] Un contenu peut être à la fois "Consommé" et "Sauvegardé".
