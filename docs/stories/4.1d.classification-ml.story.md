...
**so that** le scoring soit précis même si la source couvre plusieurs domaines.

## Acceptance Criteria

1. **Service `ClassificationService`** créé avec modèle CamemBERT
2. **Modèle ML hébergé** sur Railway (~200MB RAM additionnel)
3. **Intégration pipeline sync RSS** : classification asynchrone, non-bloquante
4. **Latence** : Classification < 300ms par article
5. **Précision** : Validée > 80% sur échantillon test (100 articles annotés)
6. **Fallback robuste** : Si service indisponible, héritage `source.granular_topics`
7. **Nouvelle `ArticleTopicLayer`** intégrée au `ScoringEngine`
...
class ClassificationService:
    ...
    def __init__(self):
        ...
        # 50 labels en français naturel
        self.candidate_labels = [
            "intelligence artificielle", "cryptomonnaie", "changement climatique",
            "justice sociale", "féminisme", "immigration", "santé", 
            "éducation", "urbanisme", "transition énergétique",
            # ... 40 autres
        ]
        
        # Mapping label français → topic_slug
        self.label_to_slug = {
    ...
    def classify(
        ...
        """Classifie un article et retourne jusqu'à 5 topic slugs."""
        try:
            # Limite tokens pour éviter overhead
    ...
    async def sync_rss_feed(self, source: Source):
        ...
        for entry in feed.entries:
            ...
            try:
                ...
                content.topics = source.granular_topics  # Fallback héritage
...
class ArticleTopicLayer(BaseScoringLayer):
    """Scoring granulaire basé sur les topics d'article."""
    ...
    def score(self, content: Content, context: ScoringContext) -> float:
        ...
        # Récupérer user_subtopics depuis context
        ...
        if matched_topics:
            # +40 pts par topic matché, max 2 topics
            ...
            context.add_reason(
                ...
                self.name, 
                match_score, 
                f"Topics précis: {', '.join(list(matched_topics)[:2])}"
            )
...
class RecommendationService:
    def __init__(self, session: AsyncSession):
        ...
        self.scoring_engine = ScoringEngine([
            ...
            QualityLayer(),
            VisualLayer(),
            ArticleTopicLayer()  # ← NOUVEAU
        ])
```

### Mise à jour ScoringContext
...
class ScoringContext:
    def __init__(
        ...
        user_subtopics: Set[str],  # ← NOUVEAU
...
def test_classification_ai_article():
    ...
    title = "ChatGPT-5 annoncé par OpenAI"
    description = "La nouvelle version du modèle de langage..."
...
### 2. Échantillon Test (Annotation Manuelle)

```python
# scripts/create_test_sample.py

# Annoter manuellement 100 articles variés
test_sample = [
    {
        "title": "La COP28 se clôt sur un accord historique",
        "description": "...",
...
async def test_fallback_on_ml_failure(monkeypatch):
    ...
    assert content.topics == ["ai", "crypto"]  # Héritage
```
...
services:
  - name: api
    env:
      ML_ENABLED: true
    memory: 768  # +200MB pour modèle (était 512)
```
...
- **Bloqué par :** Story 4.1c (taxonomie doit exister)
- **Bloque :** Story 8.3-8.6 (progression dépend de topics précis)

## Risks & Mitigation

| Risque | Probabilité | Impact | Mitigation |
|--------|-------------|--------|------------|
| RAM > 300MB | Moyen | Coût Railway | Monitorer, downgrade modèle si besoin |
| Précision < 80% | Faible | UX | Fine-tuning ou règles hybrides |
