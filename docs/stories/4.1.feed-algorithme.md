...
4. **Boost de Qualité (FQS)** : Les sources avec un `reliability_score` élevé ("High") reçoivent un bonus de visibilité (+20%). [New]
...
- **CoreLayer (V1)** : Logique de base (Match thématique + Sources suivies + Decay temporel simple).
...
- **QualityLayer (New)** : Encourage la consommation de sources fiables.
    - `ReliabilityScore.HIGH` = Bonus Score x 1.2
    - `ReliabilityScore.LOW` = Malus Score x 0.8
...
- **Validation Script** : `scripts/validate_algo.py` pour simuler des profils et vérifier les recommandations.
