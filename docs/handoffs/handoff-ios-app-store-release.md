...
| 2026-06-13 | Sync `main` → release + bump +7 | Release était figée au 11/06 (`ef7749d`, `1.0.0+6`) ; `main` (`eb1598f`) avait **9 commits d'avance** (features mobile : lettres 3-4 #833, Progression #832, polish Notif #830, Essentiel #829, carrousel sources discrètes #827). **Agent :** `git merge origin/main` dans `release/ios-app-store-2026-06-08` → **PROPRE, aucun conflit** (merge commit `487d29fd`). Le delta iOS est intégralement préservé (vérifié : `NSMicrophoneUsageDescription` + `ITSAppUsesNonExemptEncryption` dans Info.plist, 3 gardes `RevenueCatConstants.isConfigured` dans main.dart, bundle id `app.facteur`, `codemagic.yaml`). Aucun fichier `ios/` ni dépendance pubspec touché par les 9 commits → pas de nouveau motif ITMS. Bump build number **`1.0.0+6` → `1.0.0+7`** (le +6 est déjà uploadé). **⚠️ Finding hors-scope :** `origin/main` porte **2 heads Alembic** (`au01_api_usage_events` #818 + `b75132e0c6b5` event_rsvps #828, tous deux sur `gr02`, sans migration de merge) → le `Dockerfile` joue `alembic upgrade head` (singulier) qui échoue en multi-head : les migrations staging risquent de ne pas s'appliquer. **N'affecte PAS l'iOS** (Codemagic build Flutter only ; release non déployée sur Railway ; l'app vise l'API `facteur-production`). À traiter séparément sur `main` (migration de merge). **Reco hors-scope :** migrer `codemagic.yaml` dans `main` pour que les futures maj iOS = « build main » (chantier séparé). **Next PO :** push branche release (device flow) puis Start build Codemagic `iOS Release`. | Agent |
