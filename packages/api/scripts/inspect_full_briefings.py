...
async def inspect_full_briefings():
    ...
    async with async_session() as session:
        ...
        for item in items:
            if item.user_id != current_user:
                if current_user:
                    print(f"User {current_user}: Ranks={[i.rank for i in user_items]}, Contents={[str(i.content_id)[:8] for i in user_items]}")
...
if __name__ == "__main__":
    asyncio.run(inspect_full_briefings())
