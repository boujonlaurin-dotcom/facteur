...
@router.post("/follow", response_model=TopicProgressResponse, status_code=status.HTTP_201_CREATED)
async def follow_topic(
    ...
    stmt = select(UserTopicProgress).where(
        UserTopicProgress.user_id == user_uuid,
        UserTopicProgress.topic == request.topic
    )
    result = await db.execute(stmt)
...
async def get_quiz(
    ...
    stmt = select(TopicQuiz).where(TopicQuiz.topic == topic).order_by(func.random()).limit(1)
    result = await db.execute(stmt)
