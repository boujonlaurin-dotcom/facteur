...
class ContentService:
    ...
    async def update_content_status(
        ...
        if update_data.status == ContentStatus.CONSUMED:
            ...
            await self._adjust_interest_weight(
                user_id, content_id, update_data.time_spent_seconds
            )
    ...
    async def _adjust_interest_weight(
        ...
        content = await self.session.get(
            Content, content_id, options=[selectinload(Content.source)]
        )
        ...
        stmt = (
            ...
            .on_conflict_do_update(
                ...
                set_={
                    ...
                    "note_updated_at": now,
                    "is_saved": True,
                    "saved_at": func.coalesce(UserContentStatus.saved_at, now),
                    "updated_at": now,
                },
            )
            .returning(UserContentStatus)
