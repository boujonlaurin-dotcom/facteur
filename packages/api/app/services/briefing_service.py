...
class BriefingService:
    ...
    async def get_or_create_briefing(self, user_id: UUID) -> list[dict]:
        ...
        stmt = (
            ...
            .where(DailyTop3.user_id == user_id, DailyTop3.generated_at >= today_start)
        ...
        if rows:
            ...
            if len(valid_rows) < len(rows):
                logger.warning(
                    "briefing_missing_content_detected",
                    user_id=str(user_id),
                    total=len(rows),
    ...
    async def generate_briefing_for_user(
        self, user_id: UUID, global_context: dict = None
    ) -> list[dict]:
        ...
        context = ScoringContext(
            ...
            user_interest_weights={
                i.interest_slug: i.weight for i in user_profile.interests
            },
            followed_source_ids=followed_source_ids,
            user_prefs={
                p.preference_key: p.preference_value for p in user_profile.preferences
            },
        ...
        filtered_candidates = [
            c
            for c in candidates
            if (c.source and c.source.theme in user_interests)
            or (c.source_id in followed_source_ids)
        ]
        ...
        for content in filtered_candidates:
            try:
                ...
                logger.warning(
                    "scoring_failed", content_id=str(content.id), error=str(e)
                )
                pass
    ...
    async def _build_global_context(self) -> dict:
        ...
        return {"une_ids": une_ids, "trending_ids": trending_ids}
