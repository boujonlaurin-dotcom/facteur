...
class StaticPreferenceLayer(BaseScoringLayer):
    ...
    def score(self, content: Content, context: ScoringContext) -> float:
        ...
        if format_pref:
            ...
            if (
                format_pref == "audio"
                and content.content_type == ContentType.PODCAST
                or format_pref == "video"
                and content.content_type == ContentType.VIDEO
            ):
                bonus = 20.0
                triggered = True
            elif (
            ...
            if triggered:
                score += bonus
                context.add_reason(
                    content.id, self.name, bonus, f"Pref: {format_pref} match"
                )
