...
class CurationAnnotation(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "content_id", "feed_date", name="uq_curation_user_content_date"),
        CheckConstraint("label IN ('good', 'bad', 'missing')", name="ck_curation_label"),
    ...
    annotated_by: Mapped[str] = mapped_column(String(50), server_default="admin", nullable=False)
