...
class DigestRecommendationReason(BaseModel):
    ...
    breakdown: list[DigestScoreBreakdown] = Field(
...
class DigestItem(BaseModel):
    ...
    rank: int = Field(..., ge=1, description="Position in digest")
    reason: str = Field(
        ..., description="Selection reason for display (backward compatibility)"
    )
...
class DigestResponse(BaseModel):
    ...
    items: list[DigestItem] = Field(
        default_factory=list,
        description="Flat list of digest items (always populated for backward compat)",
    )
    topics: list[DigestTopic] = Field(
        default_factory=list,
        description="Topic groups (populated when format_version=topics_v1)",
    )
    completion_threshold: int = Field(
        default=5, description="Number of interactions needed to complete the digest"
    )
...
class DigestAction(StrEnum):
    ...
    NOT_INTERESTED = (
        "not_interested"  # User is not interested (triggers personalization mute)
    )
...
class DigestCompletionResponse(BaseModel):
    ...
    closure_time_seconds: int | None = None
