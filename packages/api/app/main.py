...
# Cf. docs/bugs/bug-infinite-load-requests.md. Les PR #396/#397 ont borné
# `/digest/both` et le startup catchup, mais d'autres endpoints (`/feed`,
# `/digest/generate`, writes diverses) n'avaient toujours aucun timeout. Un seul
# endpoint qui hang sur un upstream (Mistral, Google News, Supabase) bloque sa
# connexion DB ; le pool (20 max) se remplit ; chaque nouvelle requête attend
# alors `pool_timeout` (30 s) → symptôme "tout l'API charge à l'infini" côté
# mobile, même si seule une minorité d'endpoints est réellement coincée.
#
