...
def test_master_csv_quality():
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        for i, row in enumerate(reader, 1):
            ...
            assert status in ["ARCHIVED", "INDEXED", "CURATED"], (
            ...
            if status == "CURATED":
                assert row.get("Rationale"), (
                    f"Missing Rationale for curated source: {name}"
                )
                assert row.get("Score_Independence"), (
                    f"Missing Score_Independence for: {name}"
                )
